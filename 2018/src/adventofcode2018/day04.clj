...
(defn get-id [some-string]
  (-> (s/split some-string #"\s")
      (nth 3)
      (s/replace-first "#" "")))

(defn get-guards [input]
  (->> (filter #(s/includes? % "#") input)
       (map get-id)))

(defn maxsleep [input]
  (let [guards (get-guards input)
        sorted-input (sort input)])
  0)

(defn part2 [input] "")

(def input ["[1518-11-13 00:04] Guard #2411 begins shift"
