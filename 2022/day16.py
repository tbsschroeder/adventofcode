...
if __name__ == "__main__":
    lines = [re.split('[\\s=;,]+', x) for x in get_input('./input/input16.txt', '\n')]
    G = {x[1]: set(x[10:]) for x in lines}
    F = {x[1]: int(x[5]) for x in lines if int(x[5]) != 0}
    X = {x: 1 << i for i, x in enumerate(F)}
    ...
    def visit(v, budget, state, flow, answer):
        ...
        for u in F:
            ...
            if X[u] & state or newbudget <= 0:
                continue
            visit(u, newbudget, state | X[u], flow + newbudget * F[u], answer)
        return answer
