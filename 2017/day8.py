...
regs = {r.split(' ')[0]: 0 for r in instructions}
...
def not_equals(reg, no):
    return reg != no


def lower_equals(reg, no):
    return reg <= no


def greater_equals(reg, no):
    return reg >= no


def lower_than(reg, no):
    return reg < no


def greater_than(reg, no):
    return reg > no
...
for inst in instructions:
    # 0: register    1: inc/dec  2: amount   3: if
    # 4: register    5: comparison  6: condition
    tmp = inst.split(' ')
    # if eval('{} {} {}'.format(regs[tmp[4]], tmp[5], regs[6])):
    if maps_ops[tmp[5]](regs[tmp[4]], int(tmp[6])):
        if tmp[1] == 'inc':
            regs[tmp[0]] += int(tmp[2])
        else:
            regs[tmp[0]] -= int(tmp[2])
    sorted_regs = sorted(regs.items(), key=operator.itemgetter(1))
    tmp_max = sorted_regs[-1][1]
    if tmp_max > all_max:
        all_max = tmp_max

sorted_regs = sorted(regs.items(), key=operator.itemgetter(1))

print('First solution: {}'.format(sorted_regs[-1][1]))
print('Second solution: {}'.format(all_max))
