...
towers = {}  # tower -> children list
parents = {}  # child -> parent
weights = {}  # tower -> weight

for t in map(str.strip, input.split('\n')):
    # [0] = name (weight), [1] = towers
    tmp = t.split(' -> ')

    # [0] = name, [1] = (weight)
    tower_info = tmp[0].split(' ')
    children = tmp[1].split(', ') if len(tmp) > 1 else []
    towers[tower_info[0]] = children
    parents.update({c: tower_info[0] for c in children})

    # prepend 0 to have not-empty input on empty weights
    weight = tower_info[1].replace('(', '0').replace(')', '')
    weights[tower_info[0]] = int(weight)

# search parent of first tower, cause we got a tree
bottom = list(towers.keys())[0]
while bottom in parents:
    bottom = parents[bottom]

print('First solution: {}'.format(bottom))


# recursion ftw
def get_weight(tower):
    w = 0
    if tower in towers and len(towers[tower]) > 0:
        w += sum([get_weight(t) for t in towers[tower]])
    return w + weights[tower]


# searching the row without a fatty
balance = {get_weight(child): child for child in towers[bottom]}
fatty = balance[max(list(balance.keys()))]
while len(list(balance.keys())) > 1:
    balance = {get_weight(child): child for child in towers[fatty]}
    fatty = balance[max(list(balance.keys()))]

# parent of fatty is the real fatty
parent_of_normal = parents[fatty]

# whats the sizes of the real fattys siblings?
if parent_of_normal in parents:
    siblings = towers[parents[parent_of_normal]]
else:
    l = set([get_weight(t) for t in towers[parent_of_normal]])
    print('Error: {} has no parent and its children are balanced by: {}'.format(parent_of_normal, l))
    import sys
    sys.exit(1)

# whats the sizes of the real fattys siblings tree?
siblings_weights = [get_weight(sibling) for sibling in siblings]

we = weights[parent_of_normal]
ma = max(siblings_weights)
mi = min(siblings_weights)
print('Second solution: {} ({}), reduced {}'.format(parent_of_normal, we - (ma - mi), ma - mi))
