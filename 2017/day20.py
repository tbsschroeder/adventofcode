...
input = """p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
...
def init():
    buffer = list(map(str.strip, input.split('\n')))
    for index, line in enumerate(buffer):
        vals = line.split(', ')
        p = list(map(int, vals[0][3:-1].split(',')))
        v = list(map(int, vals[1][3:-1].split(',')))
        a = list(map(int, vals[2][3:-1].split(',')))
        particles[index] = {
            'p': p,
            'v': v,
            'a': a
...
def get_dist(particle):
    s = 0
    for val in particle:
        s += sum(map(abs, particle[val]))
    return s
...
def get_closest():
    ...
    while change:
        ...
        for i in particles:
            ...
            d = get_dist(particles[i])
            if d < closest['d']:
                closest['i'] = i
                has_changed = True
        change = change and has_changed


closest = {'i': 0, 'd': 9999999}
init()
get_closest()
print('First solution: {}'.format(closest['i']))


def resolve_collisions():
    collides = []
    for p in combinations(particles, 2):
        if particles[p[0]]['p'] == particles[p[1]]['p']:
            collides.append(p[0])
            collides.append(p[1])

    collides = list(set(collides))
    for c in collides:
        del particles[c]
...
print('Second solution: {}'.format(len(particles)))
