...
passphrases = [line.split(' ') for line in input.split('\n')]

valid = sum([1 for row in passphrases if len(set(row)) == len(row)])
print('First solution: {}'.format(valid))


def has_anagramm(row):
    for x, y in list(combinations(row, 2)):
        if Counter(x) == Counter(y):
            return True
    return False


valid = sum([1 for row in passphrases if not has_anagramm(row)])
print('Second solution: {}'.format(valid))
